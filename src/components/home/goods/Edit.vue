    ...
    <el-card>
      ...
      >
        <el-tab-pane label="基本信息" name="0">
          ...
          >
            <el-form-item prop="goods_name" :label="`商品名称：`">
              <el-input
                :placeholder="`请输入商品名称`"
                v-model="editShopForm.goods_name"
                clearable
              >
              </el-input>
            </el-form-item>
            ...
            <el-form-item prop="goods_cat" :label="`商品分类：`">
              <el-cascader
                v-model="editShopForm.goods_cat"
                :options="cateOptions"
                :props="cateProps"
                clearable
                placeholder="请选择商品分类"
        ...
        <el-tab-pane label="商品内容" name="4">
          <quill-editor v-model="editShopForm.goods_introduce"> </quill-editor>
          <!-- 编辑商品 -->
          <el-button type="primary" @click="editShop()" class="editShop"
            >编辑商品</el-button
          >
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  data() {
    return {
      ...
      // 编辑商品的id
      editShopId: this.$route.query.goods_id,
  ...
  methods: {
    ...
    async getShopData() {
      // 发送请求
      let { data: res } = await this.$http.get(`/goods/${this.editShopId}`);
      // console.log(res);
      if (res.meta.status == 200) {
        ...
        // 级联的id转为数组
        this.editShopForm.goods_cat = this.editShopForm.goods_cat.split(",");

        // 再遍历数组转为number
        // // for of 无法改变数组
        // // for (let item of this.editShopForm.goods_cat) {
        // //   console.log(item);
        // //   item -= 0;
        // // }
        // for in 可以改变数组
        for (let i in this.editShopForm.goods_cat) {
          this.editShopForm.goods_cat[i] -= 0;
        }

        // console.log(this.editShopForm.goods_cat);
      } else {
    ...
    checkoutCateIdChange() {
      // console.log(this.editShopForm.goods_cat);
      // 判断选中的id是否不为空 发送请求
      if (this.editShopForm.goods_cat.length != 3) {
        // 选中的不是三级分类删除数组id
        this.editShopForm.goods_cat = [];
        // 重置动态参数和静态属性数据
        this.manyData = [];
        this.onlyData = [];
        // console.log(this.editShopForm.goods_cat);
    ...
    async getManyData() {
      ...
      let { data: res } = await this.$http.get(
        `categories/${
          this.editShopForm.goods_cat[this.editShopForm.goods_cat.length - 1]
        }/attributes`,
        {
          params: {
            sel: "many",
    ...
    handleSuccess(response, file, fileList) {
      ...
      // 清空请求的临时路径数组
      this.editShopForm.pics = [];

      // 遍历图片列表数组把每一项的tmp_path拿出来
      this.fileList.forEach((item) => {
        // 临时路径数组重新编辑
        this.editShopForm.pics.push({
          pic: item.tmp_path,
        });
      });
      // console.log(this.editShopForm.pics);
      ...
      this.$refs.editShopRef.validate(async (valid) => {
        if (valid) {
          ...
          let { data: res } = await this.$http.put(
            `/goods/${this.editShopId}`,
            {
              goods_name: this.editShopForm.goods_name,
              goods_price: this.editShopForm.goods_price,
              goods_weight: this.editShopForm.goods_weight,
              goods_number: this.editShopForm.goods_number,
              goods_cat: this.editShopForm.goods_cat.join(","),
              attrs: this.editShopForm.attrs,
              pics: this.editShopForm.pics,
              goods_introduce: this.editShopForm.goods_introduce,
            }
          );
          // console.log(res);
